...
function formularioInfoGeneral(){
    document.getElementById('vista').src='situacionPatrimonial/1-datosGenerales.html';
}
function formularioDatoCurricular(){
    document.getElementById('vista').src='situacionPatrimonial/2-datosCurriculares.html';
}
function formularioDatoEncargoAct(){
    document.getElementById('vista').src='situacionPatrimonial/3-datosEmpleo.html';
}
function formularioExpLaboral(){
    document.getElementById('vista').src='situacionPatrimonial/4-experienciaLaboral.html';
}
function formularioDatoConyuge(){
    document.getElementById('vista').src='situacionPatrimonial/5-datosPareja.html';
}
function formularioDatoDependiente(){
    document.getElementById('vista').src='situacionPatrimonial/6-datosDependientes.html';
}
function formularioIngresosNetos(){
    document.getElementById('vista').src='situacionPatrimonial/7-ingresosNetos.html';
}
function formularioServidorAnterior(){
    document.getElementById('vista').src='situacionPatrimonial/8-servidorPubAnterior.html';
}
function formularioBienesInmuebles(){
    document.getElementById('vista').src='situacionPatrimonial/9-bienesInmuebles.html';
}
function formularioBienesMuebles(){
    document.getElementById('vista').src='situacionPatrimonial/10-vehiculos.html';
}
function formularioBienesMueblesNoRegistrable(){
    document.getElementById('vista').src='situacionPatrimonial/11-bienesMuebles.html';
}
function formularioInversionCuentas(){
    document.getElementById('vista').src='situacionPatrimonial/12-inversionesCuentasValores.html';
}
...
function formularioUsoPropiedad(){
    document.getElementById('vista').src='situacionPatrimonial/14-prestamoTerceros.html';
}

////////////////////////////// Cambio de Formularios Declaracion de Intereses //////////////////////////////
function formularioEmpresa(){
    document.getElementById('vista').src='situacionIntereses/1-participacionEmpresas.html';
}
function formularioInstituciones(){
    document.getElementById('vista').src='situacionIntereses/2-participacionInstituciones.html';
}
function formularioApoyoPublico(){
    document.getElementById('vista').src='situacionIntereses/3-beneficiosPublicos.html';
}
function formularioReprActiva(){
    document.getElementById('vista').src='situacionIntereses/4-representacion.html';
}
function formularioClientesPrincipales(){
    document.getElementById('vista').src='situacionIntereses/5-clientesPrincipales.html';
}
function formularioBeneficiosGratis(){
    document.getElementById('vista').src='situacionIntereses/6-beneficiosPrivados.html';
}
function formularioFideicomisos(){
    document.getElementById('vista').src='situacionIntereses/7-fideicomisos.html';
}
...
function descarga(){
    ...
    xhr.open('GET', ip + '/declaraciones/public/publicViewDeclaracionReport?id_declarante=' + sessionStorage.getItem("id") + '&tipo_declaracion=' + sessionStorage.getItem("tipo_declaracion") + '&id_declarador=' + sessionStorage.getItem("id"), true);
...
function entes(){
    $.ajax({
        ...
        for(var i=0;i<Object.keys(data['result']['data']).length;i++){
            ...
            if("SECRETARIA EJECUTIVA DEL SISTEMA ESTATAL ANTICORRUPCION" == option.text){
...
function buscar(){
    ...
    $.ajax({
        ...
        var tabla = "<tr><th>#</th><th>Nombre </th><th>Ente Público</th><th>Cargo Actual</th></tr>";
...
function getFuncionario(){
    $.ajax({
        url: ip + '/declaraciones/public/selectPublicJsonSNA?id_ente='+sessionStorage.getItem("ente")+'&id_busqueda='+sessionStorage.getItem("id")+'&tipo_declaracion='+sessionStorage.getItem("tipo_declaracion"),
        type: 'GET',
        dataType: 'json',
    }).done(function(datos){
...
function servidorPublico(){
    $.ajax({
        url: ip + '/declaraciones/public/selectPublicHeaderData?tipo_declaracion='+sessionStorage.getItem("tipo_declaracion")+'&id_declarador='+sessionStorage.getItem("id"),
        type: 'GET', 
        dataType: 'json',
    }).done(function(datos){
        document.getElementById('nombre_completo').innerHTML=datos.result.data.nombre_completo;
        document.getElementById('correo').innerHTML=datos.result.data.correo;
        document.getElementById('encargo').innerHTML=datos.result.data.encargo_actual;
    }).fail( function( jqXHR, textStatus, errorThrown ) {
        alert(jqXHR.responseJSON.msg);
    });
}