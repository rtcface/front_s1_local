    ...
    .done(function (datos) {
      if (datos.status) {
        ...
        document.getElementById("nacionalidad").innerHTML =
          datos.result.data[0].nacionalidad;
        document.getElementById("servidorAnterior").innerHTML = servAnt;
        document.getElementById("estado_civil").innerHTML =
          datos.result.data[0].estado_civil;
        document.getElementById("regimen_matrimonial").innerHTML =
          datos.result.data[0].regimen_matrimonial;
      } else {
        alert(datos.msg);
      }
    })
      ...
      if (datos.status) {
        ...
        for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
          ...
          contenidos =
            ...
            '<h4 id="sector_industria" style="margin: 0">' +
            datos.result.data[i].id_sector_industria +
            "</h4>" +
            "<br><br>" +
            "<label>¿Es dependiente económico?</label><br>" +
            '<h4 id="interes_sector" style="margin: 0">' +
            dependiente +
            "</h4>" +
            "</div>" +
            ' <div class="col-md-5">' +
            "<label>Proveedor o contratista de gobierno</label><br>" +
            '<h4 id="proveedor_gobierno" style="margin: 0">' +
            proveedor +
            "</h4>" +
            " <br><br>" +
            "</div>" +
      ...
      if (datos.status) {
        ...
        for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
          contenidos =
            ...
            '<div class="col-md-1"></div>' +
            '<div class="col-md-10">' +
            '<h2  style="padding: 0;text-align: left">' +
            ...
            '<h4 style="margin: 0">' +
            datos.result.data[i].nivel_curricular +
            "</h4>" +
            "</div>";
          if (
            datos.result.data[i].especialidad != null &&
            datos.result.data[i].especialidad != ""
          ) {
            contenidos =
              contenidos +
              '<div class="col-md-5">' +
          ...
          contenidos =
            ...
            '<h4 style="margin: 0"> ' +
            datos.result.data[i].fecha_obtencion_doc +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group"> ' +
            ...
            ' <h4 style="margin: 0"> ' +
            datos.result.data[i].ubicacion_institucion +
            " </h4>" +
            "</div>" +
            '</div><br><hr class="separador"><br>';
        }
  ...
  $.ajax({
    ...
    .done(function (datos) {
      if (datos.status) {
        ...
        var contrato = "No";
        for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
          var estado = datos.result.data[i].entidad_domicilio,
            municipio = datos.result.data[i].municipio_domicilio;
          ...
          contenidos =
            ...
            '<h4 style="margin: 0">' +
            datos.result.data[i].area_adscripcion +
            "</h4>" +
            "</div>" +
            '<div class="col-md-5">' +
            "<label>Nivel del empleo</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].nivel_encargo +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group">' +
            ...
            ' <h4 style="margin: 0">' +
            datos.result.data[i].funciones_principales +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group"> ' +
            ...
            '<h4  style="margin: 0">' +
            datos.result.data[i].fecha_posesion +
            "</h4>" +
            "</div>" +
            '<div class="col-md-5">' +
            "<label>Teléfono de oficina y extensión</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].tel_oficina +
            "-" +
            datos.result.data[i].ext_tel_oficina +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group"> ' +
            ...
            "<label>País</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].pais_ubicacion +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group"> ' +
            ...
            '<h4  style="margin: 0">' +
            datos.result.data[i].colonia_domicilio +
            "</h4>" +
            "</div>" +
            '<div class="col-md-5">' +
            "<label>Calle</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].nombre_via +
            "</h4>" +
            "</div>" +
            "</div><hr>" +
            '<div class="form-group"> ' +
            ...
            '<h4  style="margin: 0">' +
            datos.result.data[i].cp_domicilio +
            "</h4>" +
            "</div>" +
            '<div class="col-md-3">' +
            "<label>Número exterior</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].no_exterior_domicilio +
            "</h4>" +
            "</div>" +
            '<div class="col-md-3">' +
            "<label>Número interior</label><br>" +
            '<h4  style="margin: 0">' +
            datos.result.data[i].no_interior_domicilio +
            "</h4>" +
            "</div>" +
            '</div><br><hr class="separador"><br>';
        }
      ...
      if (datos.status) {
        ...
        for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
          contenidos =
            contenidos +
            ' <div class="form-group">' +
            '<h4 id="tiempo_encargo" style="padding: 0;text-align: left">de ' +
            datos.result.data[i].fecha_ingreso +
            " a " +
            datos.result.data[i].fecha_salida +
            "</h4><br>" +
            '<div class="col-md-1"></div>' +
            '<div class="col-md-7">' +
            '<h2 id="puesto" style="padding: 0; text-align: left">' +
            datos.result.data[i].cargo_puesto +
            "</h2>" +
            "</div>" +
            "</div><hr>";
          if (datos.result.data[i].ambito == "PUBLICO") {
            contenidos =
              ...
              '<h4 id="nivel_gobierno" style="margin: 0">' +
              datos.result.data[i].nivel +
              "</h4>" +
              "</div>" +
              '<div class="col-md-5">' +
              "<label>Poder</label><br>" +
              '<h4 id="poder" style="margin: 0">' +
              datos.result.data[i].poder +
              "</h4>" +
              "</div>" +
              "</div><hr>";
          ...
          contenidos =
            ...
            '<h4 id="institucion" style="margin: 0">' +
            datos.result.data[i].nombre_institucion +
            "</h4><br>" +
            "</div>" +
            '<div class="col-md-5">' +
            "<label>Área de adscripción</label><br>" +
            '<h4 id="unidad_administrativa" style="margin: 0">' +
            datos.result.data[i].unidad_administrativa +
            "</h4>" +
            "</div>" +
            "</div><hr>";
          if (datos.result.data[i].ambito == "PUBLICO") {
            ...
            contenidos =
              ...
              "<label>Sector / Industria</label><br>" +
              '<h4 id="unidad_administrativa" style="margin: 0">' +
              datos.result.data[i].sector_industria +
              "</h4>" +
              "</div>" +
              "</div><hr>";
          }
          contenidos =
            ...
            '<h4 style="margin: 0">' +
            datos.result.data[i].pais_domicilio +
            "</h4>" +
            "</div>" +
            '</div><br><hr class="separador"><br>';
        }
