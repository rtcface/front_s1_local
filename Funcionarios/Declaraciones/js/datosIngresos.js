...
function guardarActIndustrial(){
    var htmlTags='';
    if($("#ingresos_industrial").val()!=0 && $("#nombre_industria").val()!='' && $("#tipoNegocio").val()!=''){
        actIndust={
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaActIndust[i].razon_social+'</td>'+
                '<td>'+listaActIndust[i].tipo_negocio+'</td>'+
                '<td>'+listaActIndust[i].valor+'</td>'+
                '<td><button class="botonMenos" onclick="borrarActIndustrial('+i+')">Borrar</button></td></tr>'
        ...
        alert('Complete los datos de la actividad industrial, comercial y/o empresarial')
    }
}
...
function guardarActFinanciera(){
    var htmlTags='';
    if($('input:radio[name=tipoInstrumento]:checked') && $("#ingresos_financiera").val()>0){
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaActFinanciera[i].instrumento_valor+'</td>'+
                '<td>'+listaActFinanciera[i].valor+'</td>'+
                '<td><button class="botonMenos" onclick="borrarActFinanciera('+i+')">Borrar</button></td></tr>'
...
function guardarServiciosProf(){
    ...
    if($("#ingresos_servicios").val()>0 && $("#tipo_servicio").val()!=''){
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaServiciosProf[i].tipo_servicio+'</td>'+
                '<td>'+listaServiciosProf[i].valor+'</td>'+
                '<td><button class="botonMenos" onclick="borrarServiciosProf('+i+')">Borrar</button></td></tr>'
...
function guardarOtrosIngresos(){
    ...
    if($("#ingresos_varios").val()>0 && $("#especificar_varios").val()!=''){
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaOtrosIngresos[i].tipo_ingreso+'</td>'+
                '<td>'+listaOtrosIngresos[i].valor+'</td>'+
                '<td><button class="botonMenos" onclick="borrarOtrosIngresos('+i+')">Borrar</button></td></tr>'
...
function guardarEnajenacion(ingresos){
    var htmlTags='';
    if($('input:radio[name=tipoBienEnajenado]:checked') && $("#ingresos_enajenacion").val()>0){
        enajenacion={
            valor: $("#ingresos_enajenacion").val(),
            tipo_bien_enajenado: $('input:radio[name=tipoBienEnajenado]:checked').val()
        }
        listaEnajenacion.push(enajenacion);

        var i=listaEnajenacion.length-1;
        htmlTags=htmlTags+
            '<tr><td>'+listaEnajenacion[i].tipo_bien_enajenado+'</td>'+
                '<td>'+listaEnajenacion[i].valor+'</td>'+
                '<td><button class="botonMenos" onclick="borrarEnajenacion('+i+')">Borrar</button></td></tr>'
        ...
        data: JSON.stringify({ 
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoIngresos').style.color='#1EE164'
...
function selectIngresosNetos(){
    $.ajax({
        url: ip + '/declaraciones/declarante/selectDeclaranteIngresosNetos?tipo_declaracion=' + tipoDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                actualizar=true;
                var htmlTags='',htmlTags2='',htmlTags3='',htmlTags4='',htmlTags5='';
                sessionStorage.setItem("idIngresos",datos.result.data[0].id);
                document.getElementById('remuneracion_neta').value=datos.result.data[0].ing_neto_cargo_publico;
                document.getElementById('otros_ingresos').value=datos.result.data[0].otros_ing_decl;
                document.getElementById('total_ingresos_industrial').value=datos.result.data[0].ing_act_indust;
                if(datos.result.data[0].actividades!=null){
                    datos.result.data[0].actividades.forEach(element => {
                        htmlTags=htmlTags+ '<tr><td>'+element.razon_social+'</td>'+
                                        '<td>'+element.tipo_negocio+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',1)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaActividadInsdustrial').innerHTML=htmlTags;
                }
                document.getElementById('total_ingresos_financiera').value=datos.result.data[0].ing_act_finan;
                if(datos.result.data[0].actividades_financieras!=null){
                    datos.result.data[0].actividades_financieras.forEach(element => {
                        htmlTags2=htmlTags2+ '<tr><td>'+element.instrumento_valor+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',2)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaActividadFinanciera').innerHTML=htmlTags2;  
                }
                document.getElementById('total_ingresos_servicios').value=datos.result.data[0].ing_serv_prof;
                if(datos.result.data[0].serv_profesionales!=null){
                    datos.result.data[0].serv_profesionales.forEach(element => {
                        htmlTags3=htmlTags3+ '<tr><td>'+element.tipo_servicio+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',3)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaServicioProf').innerHTML=htmlTags3;
                }
                document.getElementById('total_ingresos_enajenacion').value=datos.result.data[0].ing_enajenacion;
                if(datos.result.data[0].bienes!=null){
                    datos.result.data[0].bienes.forEach(element => {
                        htmlTags4=htmlTags4+ '<tr><td>'+element.tipo_bien_enajenado+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',4)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaEnajenacion').innerHTML=htmlTags4;
                }
                document.getElementById('total_ingresos_varios').value=datos.result.data[0].otros_ing_desp_imp;
                if(datos.result.data[0].ingresos!=null){
                    datos.result.data[0].ingresos.forEach(element => {
                        htmlTags5=htmlTags5+ '<tr><td>'+element.tipo_ingreso+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',5)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaOtrosIngresos').innerHTML=htmlTags5;
                }
                document.getElementById('ingresos_declarante').value=datos.result.data[0].suma_ing_decl;
                document.getElementById('ingresos_pareja').value=datos.result.data[0].suma_ing_dep;
                document.getElementById('ingresos_total').value=datos.result.data[0].suma_ingresos;
                document.getElementById('obsComentarios').value=datos.result.data[0].observaciones;
                $('#formCompleto').show();
            } else{
                alert(datos.msg);
...
function insertarServidorPublico(){
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
                    "id_declarante": id_declarante,
                    "tipo_declaracion": tipoDeclaracion,
                    "fecha_inicio": document.getElementById('fecIngreso').value.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3/$2/$1'),
                    "fecha_fin": document.getElementById('fecSalida').value.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3/$2/$1'),
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoAnteriorServidor').style.color='#1EE164'
                    window.location.href='#tituloD';
                    location.reload();
                }else{
                    window.parent.location.reload();
...
function selectServidorPublico(){
    aplicaServidorAnt();
    $.ajax({
        url: ip + '/declaraciones/declarante/selectDeclaranteServidorAnterior?tipo_declaracion=' + tipoDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status && aplica){
                ...
                var htmlTags='',htmlTags2='',htmlTags3='',htmlTags4='',htmlTags5='';
                sessionStorage.setItem("idIngresos",datos.result.data[0].id);
                $("#fecIngreso").datepicker( "setDate" , new Date(datos.result.data[0].fecha_inicio.split("/").reverse().join("/")));
                $("#fecSalida").datepicker( "setDate" , new Date(datos.result.data[0].fecha_fin.split("/").reverse().join("/")));
                document.getElementById('remuneracion_neta').value=datos.result.data[0].remun_servicio_pub;
                document.getElementById('otros_ingresos').value=datos.result.data[0].otros_ing;
                document.getElementById('total_ingresos_industrial').value=datos.result.data[0].ing_act_indust;
                if(datos.result.data[0].actividades!=null){
                    datos.result.data[0].actividades.forEach(element => {
                        htmlTags=htmlTags+ '<tr><td>'+element.razon_social+'</td>'+
                                        '<td>'+element.tipo_negocio+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',6)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaActividadInsdustrial').innerHTML=htmlTags;   
                }
                document.getElementById('total_ingresos_financiera').value=datos.result.data[0].ing_act_finan;
                if(datos.result.data[0].actividades_financieras!=null){
                    datos.result.data[0].actividades_financieras.forEach(element => {
                        htmlTags2=htmlTags2+ '<tr><td>'+element.instrumento_valor+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',7)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaActividadFinanciera').innerHTML=htmlTags2;   
                }
                document.getElementById('total_ingresos_servicios').value=datos.result.data[0].ing_serv_prof;
                if(datos.result.data[0].serv_profesionales!=null){
                    datos.result.data[0].serv_profesionales.forEach(element => {
                        htmlTags3=htmlTags3+ '<tr><td>'+element.tipo_servicio+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',8)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaServicioProf').innerHTML=htmlTags3;   
                }
                document.getElementById('total_ingresos_enajenacion').value=datos.result.data[0].ing_enajenacion;
                if(datos.result.data[0].bienes!=null){
                    datos.result.data[0].bienes.forEach(element => {
                        htmlTags4=htmlTags4+ '<tr><td>'+element.tipo_bien_enajenado+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',9)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaEnajenacion').innerHTML=htmlTags4;
                }
                document.getElementById('total_ingresos_varios').value=datos.result.data[0].otros_ing_desp_imp;
                if(datos.result.data[0].ingresos!=null){
                    datos.result.data[0].ingresos.forEach(element => {
                        htmlTags5=htmlTags5+ '<tr><td>'+element.tipo_ingreso+'</td>'+
                                        '<td>'+element.valor+'</td>'+
                                        '<td><button class="botonMenos" onclick="deleteIngresos('+element.id+',10)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaOtrosIngresos').innerHTML=htmlTags5;   
                }
                document.getElementById('ingresos_declarante').value=datos.result.data[0].ingreso_neto_declarante;
                document.getElementById('ingresos_pareja').value=datos.result.data[0].ingreso_neto_pareja_dep;
                document.getElementById('ingresos_total').value=datos.result.data[0].ing_total;
                document.getElementById('obsComentarios').value=datos.result.data[0].observaciones;
                $('#formCompleto').show();
            } else{
                marcarAplica(5);
...
function deleteIngresos(id, tBien){
    ...
    $.ajax({
        url: ip + '/declaraciones/declarante/'+borrar+'?tipo_declaracion='+tipoDeclaracion +'&id=' + id,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
