...
function guardarTerceros(){
    var htmlTags='';
    var fisica=false, moral=false;
    var tipoPersona='';
    const tags=/^([A-Z\u00d1a-z\u00f1]{3})([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])([A-Z\u00d1a-z\u00f10-9]{3})$/;
    const tags2=/^([A-Z\u00d1a-z\u00f1]{4})([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])([A-Z\u00d1a-z\u00f10-9]{3})$/;
    ...
    if( rfc_valido && $("#nombreTercero").val()!=''){
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaTercero[i].nombre+'</td>'+
                '<td>'+listaTercero[i].rfc+'</td>'+
                '<td>'+tipoPersona+'</td>'+
                '<td><button class="botonMenos" onclick="borrarTerceros('+i+')">Borrar</button></td></tr>'
...
function guardarTransmisor(){
    var htmlTags='', tipoRelacion=$("#tipoRelacion").val();
    var fisica=false, moral=false;
    var tipoPersona='';
    const tags=/^([A-Z\u00d1a-z\u00f1]{3})([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])([A-Z\u00d1a-z\u00f10-9]{3})$/;
    const tags2=/^([A-Z\u00d1a-z\u00f1]{4})([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])([A-Z\u00d1a-z\u00f10-9]{3})$/;
    ...
    if(rfc_valido && $("#nombreRazon").val()!=''){
        ...
        htmlTags=htmlTags+
            '<tr><td>'+listaTransmisor[i].nombre+'</td>'+
                '<td>'+listaTransmisor[i].rfc+'</td>'+
                '<td>'+listaTransmisor[i].relacion+'</td>'+
                '<td>'+tipoPersona+'</td>'+
                '<td><button class="botonMenos" onclick="borrarTransmisor('+i+')">Borrar</button></td></tr>'
...
function deleteTerceros(id, tBien){
    ...
    $.ajax({
        url: ip + '/declaraciones/activos/'+borrar+'?tipo_declaracion='+tipoDeclaracion +'&id=' + id,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
...
function selectBienesInmuebles(){
    ...
    var htmlTags='', htmlTags2='', tPerson='';
    $.ajax({
        url: ip + '/declaraciones/activos/selectActivosBienesInmueblesByID?tipo_declaracion='+tipoDeclaracion+'&id='+idDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                ...
                if(datos.result.data.tipo_bien!='CASA' && datos.result.data.tipo_bien!='DEPARTAMENTO'  
                    && datos.result.data.tipo_bien!='EDIFICIO' && datos.result.data.tipo_bien!='LOCAL COMERCIAL'  
                    && datos.result.data.tipo_bien!='BODEGA' && datos.result.data.tipo_bien!='PALCO'   
                    && datos.result.data.tipo_bien!='RANCHO' && datos.result.data.tipo_bien!='TERRENO' ){
                    ...
                    if(datos.result.data.terceros!=null){
                        datos.result.data.terceros.forEach(element => {
                            ...
                            '<tr><td>'+element.nombre+'</td>'+
                                '<td>'+element.rfc+'</td>'+
                                '<td>'+tPerson+'</td>'+
                                '<td><button class="botonMenos" onclick="deleteTerceros('+element.id+',1)">Borrar</button></td></tr>' 
                        });
                        document.getElementById('tablaprueba').innerHTML=htmlTags;   
                    } 
                }
                document.getElementById('porcentaje').value=datos.result.data.porcentaje_propiedad;
                document.getElementById('terreno').value=datos.result.data.superficie_terreno;
                document.getElementById('construccion').value=datos.result.data.superficie_construccion;    
                if(datos.result.data.transmisor!=null){
                    datos.result.data.transmisor.forEach(element =>{ 
                        ...
                        '<tr><td>'+element.nombre+'</td>'+
                            ...
                            '<td>'+tPerson+'</td>'+
                            '<td><button class="botonMenos" onclick="deleteTransmisor('+element.id+',1)">Borrar</button></td></tr>' 
                    });
                ...
                document.getElementById('valor').value=datos.result.data.valor_adquisicion;
                document.getElementById('tipoMoneda').value=datos.result.data.tipo_moneda;
                $("#valorAdquisicion").val(datos.result.data.tipo_operacion);
                $("#fechaAdquirio").datepicker( "setDate" , new Date(datos.result.data.fecha_adquisicion.split("/").reverse().join("/")));
                document.getElementById('datosPropiedad').value=datos.result.data.no_escritura_publica;
                ...
                if(datos.result.data.motivo_baja!='VENTA' && datos.result.data.motivo_baja!='DONACION' && datos.result.data.motivo_baja!='SINIESTRO' && datos.result.data.motivo_baja!='NO APLICA'){
                    $("#motivoBaja").val('OTRO');
...
function  insertarBienesInmuebles(){
    ...
    var colonia='', tipoVia='', nombreVia='', cp=null, noExt='', noExt='';
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
                    ...
                    "folio_real": "",
                    "fecha_contrato": document.getElementById('fechaAdquirio').value.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3/$2/$1'),
                    ...
                    "fecha_adquisicion": document.getElementById('fechaAdquirio').value.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3/$2/$1'),
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoInmuebles').style.color='#1EE164'
...
function actualizarBienesInmuebles(){
    var lugar=$('input:radio[name=lugar]:checked').val();
    var colonia='', tipoVia='', nombreVia='', cp='', noExt='', noExt='',estado='',municipio='',estadoExt='',municipioExt='';
    var motivoBaja=$("#motivoBaja").val();
    var tipoBien=$("#tipoBien").val(); 
...
function selectVehiculos(){
    ...
    var htmlTags='', htmlTags2='', tPerson='';
    $.ajax({
        url: ip + '/declaraciones/activos/selectActivosVehiculosByID?tipo_declaracion='+tipoDeclaracion+'&id='+idDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                ...
                if(datos.result.data.tipo_mueble!='AUTOMOVIL O MOTOCICLETA' && datos.result.data.tipo_mueble!='AERONAVE'  && datos.result.data.tipo_mueble!='BARCO O YATE' ){
                    ...
                    if(datos.result.data.terceros!=null){
                        datos.result.data.terceros.forEach(element => {
                            ...
                            '<tr><td>'+element.nombre+'</td>'+
                                '<td>'+element.rfc+'</td>'+
                                '<td>'+tPerson+'</td>'+
                                '<td><button class="botonMenos" onclick="deleteTerceros('+element.id+',2)">Borrar</button></td></tr>' 
                        });
                        document.getElementById('tablaprueba').innerHTML=htmlTags;   
                    }   
                }
                if(datos.result.data.transmisor!=null){
                    datos.result.data.transmisor.forEach(element =>{
                        ...
                        '<tr><td>'+element.nombre+'</td>'+
                            ...
                            '<td>'+element.relacion+'</td>'+
                            '<td>'+tPerson+'</td>'+
                            '<td><button class="botonMenos" onclick="deleteTransmisor('+element.id+',2)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaTransmisor').innerHTML=htmlTags2;
                }
                ...
                $("#formaAdquirio").val(datos.result.data.forma_adquisicion);
                $("#formaPago").val(datos.result.data.forma_pago);
                document.getElementById('valor').value=datos.result.data.precio_adquisicion;
                document.getElementById('tipoMoneda').value=datos.result.data.tipo_moneda;
                document.getElementById('fechaAdquirio').value=datos.result.data.fecha_adquisicion;
                $("#fechaAdquirio").datepicker( "setDate" , new Date(datos.result.data.fecha_adquisicion.split("/").reverse().join("/")));
                ...
                document.getElementById('numeroSerie').value=datos.result.data.numero_serie;
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                vehiculoAplica(true)
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoVehiculo').style.color='#1EE164'
                    window.location.href='#tituloD';
                    location.reload();
                }else{
                    window.parent.location.reload();
...
function selectBienesMuebles(){
    ...
    var htmlTags='', htmlTags2='', tPerson='';
    $.ajax({
        url: ip + '/declaraciones/activos/selectActivosBienesMueblesNRByID?tipo_declaracion='+tipoDeclaracion+'&id='+idDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                ...
                if(datos.result.data.tipo_bien!='MENAJE DE CASA' && datos.result.data.tipo_bien!='APARATOS ELECTRONICOS Y ELECTRODOMSTICOS'  && datos.result.data.tipo_bien!='JOYAS'
                && datos.result.data.tipo_bien!='COLECCIONABLES'  && datos.result.data.tipo_bien!='OBRAS DE ARTE' ){
                    $("#otroBien").val(datos.result.data.tipo_bien);
                    $("#tipoBien").val('OTRO');
                    $('#otroBien').show();
                }else{
                ...
                document.getElementById('descBien').value=datos.result.data.descripcion_bien;
                $("#titular").val(datos.result.data.titular);
                if(!datos.result.data.titular.includes('TERCEROS')){
                    $('#tercero').hide();
                    $('#terceroDatos').hide();
                    ...
                    if(datos.result.data.terceros!=null){
                        datos.result.data.terceros.forEach(element => {
                            ...
                            '<tr><td>'+element.nombre+'</td>'+
                                '<td>'+element.rfc+'</td>'+
                                '<td>'+tPerson+'</td>'+
                                '<td><button class="botonMenos" onclick="deleteTerceros('+element.id+',3)">Borrar</button></td></tr>' 
                        });
                        document.getElementById('tablaprueba').innerHTML=htmlTags;   
                    }  
                }
                ...
                if(datos.result.data.transmisor!=null){
                    datos.result.data.transmisor.forEach(element =>{
                        ...
                        '<tr><td>'+element.nombre+'</td>'+
                            ...
                            '<td>'+element.relacion+'</td>'+
                            '<td>'+tPerson+'</td>'+
                            '<td><button class="botonMenos" onclick="deleteTransmisor('+element.id+',3)">Borrar</button></td></tr>' 
                    });
                    document.getElementById('tablaTransmisor').innerHTML=htmlTags2;
                }
...
function  insertarBienesMuebles(){
    var tipoBien=$("#tipoBien").val(), motivoBaja=$("#motivoBaja").val();
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoMuebles').style.color='#1EE164'
                    window.location.href='#tituloD';
                    location.reload();
                }else{
                    window.parent.location.reload();
...
function selectInversiones(){
    ...
    $('#agregar_nuevo').attr('disabled','disabled');
    var htmlTags='', tPerson='';
    $.ajax({
        url: ip + '/declaraciones/activos/selectActivosInversionesCuentasByID?tipo_declaracion='+tipoDeclaracion+'&id='+idDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                ...
                if(datos.result.data.tipo_inversion=='BANCARIA'){
                    ...
                    $("#contenedorValores").hide();
                    $("#contenedorAfores").hide();
                }else if(datos.result.data.tipo_inversion=='ORGANIZACIONES PRIVADAS Y/O MERCANTILES'){
                    $("#selectOrganizaciones").val(datos.result.data.descripcion_inversion);
                    ...
                    $("#contenedorValores").hide();
                    $("#contenedorAfores").hide();
                }else if(datos.result.data.tipo_inversion=='POSESION DE MONEDAS Y/O METALES'){
                ...
                document.getElementById('numeroCuenta').value=datos.result.data.cuenta_contrato;
                $("#titular").val(datos.result.data.titular);
                if(!datos.result.data.titular.includes('TERCEROS')){
                    $('#tercero').hide();
                    $('#terceroDatos').hide();
                    ...
                    if(datos.result.data.terceros!=null){
                        datos.result.data.terceros.forEach(element => {
                            ...
                            '<tr><td>'+element.nombre+'</td>'+
                                '<td>'+element.rfc+'</td>'+
                                '<td>'+tPerson+'</td>'+
                                '<td><button class="botonMenos" onclick="deleteTerceros('+element.id+',4)">Borrar</button></td></tr>' 
                        });
                        document.getElementById('tablaprueba').innerHTML=htmlTags;   
                    }     
                }
                ...
                document.getElementById('institucion').value=datos.result.data.razon_social;
                document.getElementById('rfc').value=datos.result.data.rfc;
                document.getElementById('saldo').value=datos.result.data.saldo_correspondiente;
                document.getElementById('tipoMoneda').value=datos.result.data.tipo_moneda;
                document.getElementById('obsComentarios').value=datos.result.data.observaciones;
                $('#formCompleto').show();
            } else{
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoInversiones').style.color='#1EE164'
                    window.location.href='#tituloD';
                    location.reload();
                }else{
                    window.parent.location.reload();
...
function selectAdeudos(){
    ...
    var htmlTags='', tPerson='';
    $.ajax({
        url: ip + '/declaraciones/pasivos/selectPasivosAdeudosByID?tipo_declaracion='+tipoDeclaracion+'&id='+idDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                    ...
                    if(datos.result.data.terceros!=null){
                        datos.result.data.terceros.forEach(element => {
                            ...
                            '<tr><td>'+element.nombre+'</td>'+
                                '<td>'+element.rfc+'</td>'+
                                '<td>'+tPerson+'</td>'+
                                '<td><button class="botonMenos" onclick="deleteTerceros('+element.id+',5)">Borrar</button></td></tr>' 
                        });
                        document.getElementById('tablaprueba').innerHTML=htmlTags;   
                    }    
                }
                if(datos.result.data.tipo_adeudo!='CHIP' && datos.result.data.tipo_adeudo!='CAUT'  && datos.result.data.tipo_adeudo!='CPER'
                && datos.result.data.tipo_adeudo!='TCRN'  && datos.result.data.tipo_adeudo!='TCRD' && datos.result.data.tipo_adeudo!='PRPE'){
                ...
                $("#fechaAdquirio").datepicker( "setDate" , new Date(datos.result.data.fecha_adeudo.split("/").reverse().join("/")));
                document.getElementById('monto').value=datos.result.data.monto_original;
                document.getElementById('tipoMoneda').value=datos.result.data.tipo_moneda;
...
function  insertarAdeudos(){
    ...
    $.ajax({
        ...
        data: JSON.stringify({ 
                    ...
                    "fecha_adeudo": document.getElementById('fechaAdquirio').value.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3/$2/$1'),
            ...
            if(data.msg=='Sesión Expirada'){
                ...
                pasivosAplica(true)
                if(confirm('¿Agregar otro registro?')){
                    window.parent.document.getElementById('semaforoPasivos').style.color='#1EE164'
                    window.location.href='#tituloD';
                    location.reload();
                }else{
                    window.parent.location.reload();
