...
function verificarFecha(){
    var pasa=false;
    var fecha1=document.getElementById('fecIngreso').value.split("/").reverse().join("/");
    var fecha2=document.getElementById('fecSalida').value.split("/").reverse().join("/");
...
function obligarFinalizado(){
    ...
    var niveles=['PRIMARIA','SECUNDARIA','BACHILLERATO','CARRERA TECNICA O COMERCIAL','LICENCIATURA','ESPECIALIDAD','MAESTRIA','DOCTORADO']
    $.ajax({
        url: ip + '/declaraciones/declarante/findCurricularSemaphore?tipo_declaracion='+tipoDeclaracion,
        type: "GET",
        dataType: "json",
        headers: {
            'X-Auth-Token' : token
            ...
            if(datos.status){
                ...
                if($('#nivelCurriculo').val() == 'PRIMARIA' || $('#nivelCurriculo').val() == 'SECUNDARIA' || $('#nivelCurriculo').val() == 'BACHILLERATO' ){
                    $('#contCarrera').hide();
                }else{
                    $('#contCarrera').show();
                }
            ...
            if(jqXHR.responseJSON.msg!='Error general del sistema!Contacta al Administrador. null'){
