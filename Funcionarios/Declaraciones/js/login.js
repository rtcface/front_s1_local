...
function buscarDeclaraciones() {
  $.ajax({
    ...
    .done(function (datos, textStatus, xhr) {
      if (datos.status) {
        var tabla =
          "<tr><th>Fecha de inicio</th><th>Fecha de envio</th><th>Estatus</th><th>Declaración Privada</th><th>Declaración Pública</th><th>Acuse Recibo</th></tr>";
        var estado = "",
          fechaEnvio = "-";
        for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
          ...
          if (datos.result.data[i].status) {
            ...
            tabla =
              tabla +
              "<tr><td>" +
              datos.result.data[i].fecha_inicio +
              "</td><td>" +
              fechaEnvio +
              "</td><td>" +
              estado +
              "</td>" +
              '<td><button class="botonEnviar" onclick="descargarReporte2(' +
              datos.result.data[i].id_declarante +
              ')">Descargar</button></td>' +
              '<td><button class="botonEnviar" onclick="descargarReportePublico2(' +
              datos.result.data[i].id_declarante +
              ')">Descargar</button></td>' +
              '<td><button class="botonEnviar" onclick="descargarRecibo2(' +
              datos.result.data[i].id_declarante +
              ')">Descargar</button></td></tr>';
          }
        }
...
function mostarBoton() {
  if (
    sessionStorage.getItem("rol") == "seseaadmin" ||
    sessionStorage.getItem("rol") == "entadmin"
  ) {
    $("#botonAlta").show();
    $("#status").show();
    $("#adminOpciones").show();
    ...
    .done(function (datos) {
      ...
      for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
        tabla =
          ...
          "</td> <td>" +
          datos.result.data[i]["apellido_materno"] +
          "</td> <td>" +
          datos.result.data[i]["rfc"] +
          datos.result.data[i]["rfc_homoclave"] +
          "</td> <td><input type='radio' name='op' id='cv" +
          i +
          "' value='" +
...
function cargarEntes() {
  $.ajax({
    ...
    .done(function (datos) {
      var tabla =
        "<tr><th>Nombre del ente</th><th>Siglas</th><th>Estado</th><th>Municipio</th> <th>Seleccionar</th></tr>";
      for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
        tabla =
          tabla +
          "<tr><td>" +
          datos.result.data[i]["nombre_ente"] +
          "</td> <td>" +
          datos.result.data[i]["siglas_ente"] +
          "</td> <td>" +
          datos.result.data[i]["estado_ente"] +
          "</td> <td>" +
          datos.result.data[i]["municipio_ente"] +
          "</td> <td><input type='radio' name='opEnte' id='ente" +
          i +
          "' value='" +
          datos.result.data[i]["id"] +
          "'></td> </tr>";
  ...
  $.ajax({
    ...
    .done(function (datos) {
      ...
      document.getElementById("entidadFed").value =
        datos.result.data.estado_ente;
      domMunicipio(
        datos.result.data.municipio_ente,
        datos.result.data.estado_ente
      );
    })
    .fail(function (jqXHR, textStatus, errorThrown) {
  ...
  $("#tablaDeclarantes").DataTable({
    ...
    columns: [
      ...
      { data: "anio_fiscal", title: "Año Fiscal" },
      {
        ...
        render: function (row) {
          if (row.tipo_declaracion == 1) return "INICIAL";
          else if (row.tipo_declaracion == 2) return "MODIFICACIÓN";
          else if (row.tipo_declaracion == 3) return "CONCLUSIÓN";
          else return "";
        },
      },
      ...
      { data: "declaracion_completa", title: "Dec. Simplificada" },
      {
        ...
        render: function (row) {
          return (
            "<button style='border:none;' onclick='guardarInfo(" +
            row.id +
            "," +
            row.tipo_declaracion +
            "," +
            row.id_declarante +
            ")'><input type='radio' name='declarante'  value='" +
            row.id +
            "'></button>"
          );
        },
...
function validarDecFiscal() {
  ...
  if (
    confirm("Al dar clic aquí la Declaracón Fiscal se marcará como no válida.")
  ) {
    $.ajax({
      url:
        ip +
...
function reactivarDeclaracion() {
  ...
  if (
    confirm(
      "Al dar clic aquí se reactivara la Declaración Patrimonial y dejara de aparecer como enviada."
    )
  ) {
    $.ajax({
      url:
...
function selectEmails() {
  $.ajax({
    ...
    .done(function (datos) {
      var tabla =
        "<tr><th>Nombre</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Correo Electronico</th> <th>Seleccionar</th></tr>";
      for (var i = 0; i < Object.keys(datos.result.data).length; i++) {
        tabla =
          tabla +
          "<tr><td>" +
          datos.result.data[i]["nombre_envio"] +
          "</td> <td>" +
          datos.result.data[i]["apat_envio"] +
          "</td> <td>" +
          datos.result.data[i]["amat_envio"] +
          "</td> <td>" +
          datos.result.data[i]["correo_envio"] +
          "</td> <td><input type='radio' name='opCorreo' id='correo" +
          i +
          "' value='" +
          datos.result.data[i]["id"] +
          "'></td> </tr>";
...
function insertarEmpleo(id) {
  ...
  $.ajax({
    ...
    data: JSON.stringify({
      ...
      fecha_posesion: document
        .getElementById("fecPos")
...
function buscarFuncionario() {
  ...
  var expreg =
    /^([A-Z][AEIOUX][A-Z]{2}\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])([A-Z0-9]{3}))$/;
  if (!expreg.test(rfc)) {
...
function cambioEnte() {
  if ($("input:radio[name=op]").is(":checked")) {
    ...
    if (
      confirm(
        "Se enviara la notificación para realizar el cambio de ente público dentro del mismo nivel de gobierno."
      )
    ) {
      $.ajax({
        url:
...
function restablecerPassword() {
  $.ajax({
    ...
    .done(function (datos) {
      if (datos.status) alert(datos.result.data);
...
function allStatus2() {
  ...
  $("#tablaDeclarantes").DataTable({
    ...
    columns: [
      ...
      {
        data: null,
        title: "Información Curricular Actualizada",
        visible: false,
        render: function (row) {
          url =
            ip +
