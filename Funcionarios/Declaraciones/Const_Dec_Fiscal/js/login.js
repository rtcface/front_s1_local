...
function revisarCheck() {
  if ($("input:checkbox[name=habilita_fiscal]").is(":checked")) {
    ...
    !$("input:checkbox[name=no_habilita_fiscal]").is(":checked") &&
    !$("input:checkbox[name=habilita_fiscal]").is(":checked")
  ) {
    $("input:checkbox[name=habilita_fiscal]").removeAttr("disabled");
    $("input:checkbox[name=no_habilita_fiscal]").removeAttr("disabled");
    $("#cargaFile").attr("disabled", true);
    ...
    .done(function (datos) {
      if (datos.status) {
        ...
        if (!datos.result.data.aprobada) {
          valida =
            "Declaración Fiscal Rechazada por el Organo Interno de Control";
        }
        document.getElementById("datos_archivo").innerHTML =
          "<h5>Fecha: " +
          datos.result.data.fecha_alta +
          "<br>" +
          "Nombre del archivo: " +
          datos.result.data.nombre_archivo +
          "<br>" +
          valida +
          "</h5>";
      } else {
